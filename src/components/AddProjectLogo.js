...
import {addProjectLogo} from "../api/addProjectLogo";
...
export default class AddProjectLogo extends Component {
    ...
    async createLogo () {
        ...
        addProjectLogo(this.user.logo, this.state.id)
            .then((response) => {
                if (response.status === 200) {
                    localStorage.setItem("notificationStatus", "success");
                    localStorage.setItem("notificationMessage", "Project created");
